...
async function createResume() {
  ...
  const skills = document.getElementById("skills").value.split(",").map(skill =>
  skill.trim()).join(", ");
  const experience = document.getElementById("experience").value;

    // creating resume content dynamically
    const resumeContainer = document. 
    getElementById("resume");
    resumeContainer.innerHTML = `
      <div class="container" id="resumeCon
      tent">
            ...
            <main class="main-content">
              <section>
                <h3>Education</h3>
                <p><strong>${college}
     :</strong><br> ${describe1}</p>
                     <p><strong>${college2}
    :</strong><br> ${describe2}</p>
              </section>
              <section>
                 <h3>Skills</h3>
                 <p>${skills}</p>
      ...
      (resumeContent, { scale: 2 });
        ...
        jsPDF("p", "mm", "a4");
          ...
          * pdfWidth) / canvas.width;
                         pdf.addImage(imgData, "PNG", 0,
            0, pdfWidth, pdfHeight);
                       pdf.save("resume.pdf");       
     } 
    } catch (error) {
      console.error(
...
async function shareResume() {
  try {
    ...
    console.error(
  "Error sharing resume:", error);
  }
}


// Expose function to global window object for browser usage
